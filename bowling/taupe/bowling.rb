...
class NormalScorer
  ...
  def score(pins)
    @remaining_pins = pins
    @frame_scores = []
    while more_pins? && more_frames_in_game? do
  ...
  def more_pins?
    !@remaining_pins.empty?
  end
  
  def more_frames_in_game?
    @frame_scores.size < max_frames_per_game
  end
  
  def max_frames_per_game
    10
  end
